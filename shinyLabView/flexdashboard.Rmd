...
output$coursePlot <- renderPlotly(
  if (is.null(input$resultTable_rows_selected)) {
    return(geom_blank()) # https://ggplot2.tidyverse.org/reference/geom_blank.html :-)
  } else {
      p <- plot_ly(x = ~colnames(displayData())[2:(length(colnames(displayData()))-2)],
                   y = ~displayData()[input$resultTable_rows_selected, ][length(colnames(displayData()))],
                   type = "scatter", 
                   mode = "lines",
                   line = list(color = "transparent"),
                   showlegend = FALSE, 
                   name = "HIGH") %>%
      add_trace(x = colnames(displayData())[2:(length(colnames(displayData()))-2)],
                y = displayData()[input$resultTable_rows_selected, ][length(colnames(displayData()))-1],
                type = "scatter", 
                mode = "lines",
                fill = "tonexty", 
                ...
                name = "LOW") %>%
      add_trace(x = colnames(displayData())[2:(length(colnames(displayData()))-2)],
                y = displayData()[input$resultTable_rows_selected, 2:(length(colnames(displayData()))-2)],
                type = "scatter",
                mode = "lines",
